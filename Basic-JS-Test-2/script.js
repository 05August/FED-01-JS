...
const deleteItem = () => {
  ...
  for (i = 0; i < close.length; i++) {
    close[i].onclick = function () {
      ...
      listNotify = listNotify.filter(element => element.content !== listNotify[div.getAttribute('index')].content);
      insertNotify();
    }
  }
}
...
const insertNotify = () => {
  ...
  for (let i = 0; i < listNotify.length; i++) {
    if (isToday(new Date(listNotify[i].date[0], listNotify[i].date[1] - 1, listNotify[i].date[2]))) {
      textHtmlNotify += `<div class="item today" index="${i}">Ngày ${listNotify[i].date[2]}/${listNotify[i].date[1]}/${listNotify[i].date[0]} <p>${listNotify[i].content}</p><span class="close">x</span></span></div>`;
    } else {
